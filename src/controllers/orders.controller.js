...
// Fallback company info for orders without snapshot
...
const streamInvoice = (order, res) => {
  ...
  doc.fontSize(10)
    .fillColor(COLORS.lightGray)
    .font('Helvetica')
    .text(`#${order.orderNumber || orderId.slice(-8).toUpperCase()}`, rightMargin - 120, 72, { width: 120, align: 'right' });
  ...
  doc.fontSize(9)
    .fillColor(COLORS.mutedText)
    .font('Helvetica')
    .text('Order Number', leftMargin + 150, metaY);
  doc.fontSize(11)
    .fillColor(COLORS.text)
    .font('Helvetica-Bold')
    .text(order.orderNumber || 'N/A', leftMargin + 150, metaY + 14);
  ...
  doc.fontSize(9)
    .fillColor(COLORS.mutedText)
    .font('Helvetica')
    .text('Order Status', leftMargin + 420, metaY);
  doc.fontSize(11)
    .fillColor(COLORS.text)
    .font('Helvetica-Bold')
  ...
  items.forEach((item, idx) => {
    ...
    doc.font('Helvetica')
      .text(item.quantity.toString(), col2, y + 10);
  ...
  doc.fillColor(COLORS.text)
    .text(formatCurrency(order.subtotal), totalsValueX, y, { width: 70, align: 'right' });
  y += 20;
  ...
  doc.fontSize(14)
    .text(formatCurrency(order.totalAmount || order.subtotal), totalsValueX - 20, y + 3, { width: 90, align: 'right' });
